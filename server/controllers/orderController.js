...
module.exports.getOrder_get = async (req, res) => {
  Order.find({ _id: req.params.id }).exec((error, order) => {
    if (error) return res.status(400).json({ error });
    if (order) {
          ...
          instance.payments.capture(
            ...
            (err, paymentStatus) => {
              ...
              if (paymentStatus) {
                ...
                Order.findOneAndUpdate(condition, action).exec(
                  (err, _order) => {
                    if (err) {
                      return res.status(500).json(err);
                    }
...
module.exports.updateOrderStatus_put = async (req, res) => {
  console.log(req.body.updatedStatus.params.step);
  condition = {
    _id: req.body.updatedStatus.params.id,
    "orderStatus.type": req.body.updatedStatus.params.step,
  };
  action = {
    $set: {
  ...
  Order.findOneAndUpdate(condition, action).exec((error, order) => {
    if (error) {
      console.log(error);
...
module.exports.getUserOrdersByUserId_get = async (req, res) => {
  Order.find({ user: req.query.userId }).exec((error, orders) => {
    if (error) {
      return res.status(400).json({ error });
    }
    if (orders) {
