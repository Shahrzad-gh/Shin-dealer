...
module.exports.addProduct_post = async (req, res) => {
  const { name, count, price, description, reviews, offer, category } =
    req.body;
  let picture = { img: "", id: "" };
...
module.exports.getProductsByCategory_get = async (req, res) => {
  Product.find({ category: req.params.id }).exec((error, products) => {
    if (error) return res.status(400).json({ error });
    if (products) {
      res.status(200).json({ products });
    }
...
module.exports.getproductsByCategoryASD_get = (req, res) => {
  Product.find({ category: req.query.category }).exec((error, products) => {
    if (error) return res.status(400).json({ error });
    if (products) {
      products.sort((a, b) => {
